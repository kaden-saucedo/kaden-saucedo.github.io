...
<aside class="margin-math margin-math-left">
  ...
  - Let $\pi:X\times Y\to Y$ denote a trivial family of $X$ parametrized by $Y$. If we ignore the boundary, then $\kappa_0=(K_{X\times Y/Y})^{\dim X}$. Now, $K_{X\times Y/Y}=K_{X\times Y}-\pi^\ast(K_Y)=\rho^\ast(K_X)+\pi^\ast(K_Y)-\pi^\ast(K_Y)=\rho^\ast(K_X)$. Then $\kappa_0=(K_{X\times Y/Y})^{\dim X}=(\rho^\ast(K_X))^{\dim X}=K_X^{\dim X}$. 
